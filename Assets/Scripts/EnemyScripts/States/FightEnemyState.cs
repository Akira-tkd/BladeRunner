...
public class FightEnemyState : EnemyState
{
    ...
    private const float MinAttackSpan = 10.0f;  // 再攻撃までの最短時間
    private const float MaxAttackSpan = 20.0f;  // 再攻撃までの最長時間
    private const float MinAngleChange = -10.0f;  // 一回の移動の最小角度
    private const float MaxAngleChange = 10.0f;  // 一回の移動の最大角度
